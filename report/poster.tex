...
{\large [Author Name]\quad$\bullet$\quad Baanwit Academy\quad$\bullet$\quad 2026 \quad$\bullet$\quad \textit{Phase 1 --- all six pillars evaluated}}\\[0.3ex]
...
\begin{block}{7. Mechanism tests --- energy \& angle}
\begin{keybox}
\good{CL-3 Energy} (40--120\,keV): signal \textbf{strongest at low energy} (40\,keV vs higher, $p{=}0.0001$) --- photoelectric; baseline SPR \textbf{decreases monotonically} ($\rho{=}-1.00$). \good{Mechanism supported} (fine curve noisy at $N{=}10$).\\[4pt]
\good{CL-6 Beam-angle} ($\pm30^\circ$): signal \textbf{robust across angles} (correct sign) and \textbf{strengthens with tilt} toward tangential ($|\Delta$SPR$|$: $3.4\!\to\!6.8$). \pend{Suggestive} but not conclusive at $N{=}10$.
\end{keybox}
\end{block}
...
\begin{itemize}
  ...
  \item Energy dependence \good{supports} the mechanism; angle dependence is \textbf{suggestive} --- both consistent with bone-attenuation physics.
\end{itemize}
\end{block}
...
\textcolor{ok}{\Large\checkmark} & \textcolor{warn}{\Large$\sim$}\\
\end{tabular}
\end{block}
